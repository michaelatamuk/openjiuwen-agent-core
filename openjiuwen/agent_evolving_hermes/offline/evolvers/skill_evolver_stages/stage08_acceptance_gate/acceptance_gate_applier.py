...
def apply_acceptance_gate(improvement: float,
    ...
    if not accepted:
        ...
        console.print(f"[yellow]⚠ Improvement {improvement:+.4f} < threshold {min_improvement:+.4f} "
        ...
        if cross_run_delta is not None:
            ...
            console.print(f"[{delta_color}]Cross-run delta vs prior evolved: "
                          f"{cross_run_delta:+.4f}[/{delta_color}]")
    else:
        if improvement < 0:
            console.print(f"[yellow]⚠ Improvement {improvement:+.4f} is negative "
