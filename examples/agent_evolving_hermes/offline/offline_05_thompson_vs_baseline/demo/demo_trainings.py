...
class DemoTrainings:
    ...
    def run(self, params: DemoParams) -> DemoTrainingsResults:
        ...
        # ── no_ts ─────────────────────────────────────────────────────────────
        scores_no_ts, metrics_no_ts, last_out_no_ts = self.run_no_ts(params)
        if scores_no_ts:
            runs.append(("No-TS", last_out_no_ts))

        # ── l2_l3 ─────────────────────────────────────────────────────────────
        scores_l2_l3, metrics_l2_l3, last_out_l2_l3 = self.run_l2_l3(params)
        if scores_l2_l3:
            runs.append(("L2+L3", last_out_l2_l3))

        # ── l2_only ───────────────────────────────────────────────────────────
        scores_l2, metrics_l2, last_out_l2 = self.run_l2_only(params)
        if scores_l2:
            runs.append(("L2-only", last_out_l2))

        # ── l3_only ───────────────────────────────────────────────────────────
        scores_l3, metrics_l3, last_out_l3 = self.run_l3_only(params)
        if scores_l3:
    ...
    def run_no_ts(self, params: DemoParams):
        ...
        if "no_ts" in self._config.run_modes:
            for i in range(1, self._config.n_runs + 1):
                ...
                m = step_02_run_gepa_without_ts(params.skills_root, params.skill_name, self._config.model,
    ...
    def run_l2_l3(self, params: DemoParams):
    ...
    def run_l2_only(self, params: DemoParams):
    ...
    def run_l3_only(self, params: DemoParams):
