...
# coding: utf-8
"""
demo.py — Skill Recommender quick-start example

Creates a small in-memory scoring matrix (no real GEPA run required),
loads it through the standard skill_recommender stack, and runs three
sample queries so you can see the output format right away.

Run
---
    python demo.py
    python -m skill_recommender.demo   # equivalent
"""
...
def _write_demo_matrix(directory: Path) -> None:
    """Write synthetic scoring_matrix_*.json files into *directory*."""
    for entry in _MATRIX_DATA:
        fname = f"scoring_matrix_{entry['run_id']}.json"
        with open(directory / fname, "w", encoding="utf-8") as fh:
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="skill_recommender_demo_") as tmpdir:
        ...
        # 1. Write the synthetic matrix
        _write_demo_matrix(oracle_dir)
        print(f"\n  Demo matrix written to: {oracle_dir}")

        # 2. Build the recommender (TF-IDF, baseline variant)
        ...
        for query in sample_queries:
            results = rec.recommend(
                query           = query,
                sim_threshold   = 0.10,   # lower than default: demo corpus is small
