...
Two variants are supported:
  "baseline" — baseline skill outputs cross-scored across all metrics
  "evolved"  — best-candidate (proxy evolved skill) outputs, one per source metric
...
def _load_one(path: Path, key: str) -> list[dict]:
    """Return flattened rows from one scoring_matrix_*.json for the given key."""
    try:
...
def load_matrix(
    ...
    pd.DataFrame  with columns:
        ...
        candidate_output, [source_metric,] score_<m>, norm_<m>, …
    """
    ...
    if not all_rows:
        raise ValueError(
            f"No {variant} cross-eval data found in {oracle_dir}.\n"
            "Run gepa_scoring_matrix mode with the latest step_matrix_run_gepa.py "
            "to generate baseline_cross_eval / evolved_cross_eval data."
        )
